...
console.log(
  envFileExists
    ? `INFO: 使用 ${envFile} 作为环境变量的配置文件。`
    : `INFO: ${envFile} 文件未找到。`,
);

const envFileFallback =
  /\.local$/.test(envFile) && envFile.replace(/\.local$/, '');

const envFileFallbackExists =
  !!envFileFallback && fs.existsSync(getFullPath(envFileFallback));

if (envFileFallbackExists) {
  console.log(
    `INFO: 未在 ${envFile} 中定义的环境变量将使用 ${envFileFallback} 中对应的变量代替。`,
  );
}
